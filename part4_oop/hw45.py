...
class LFUPolicy(Policy[K]):
    ...
    def get_key_to_evict(self) -> K | None:
        if len(self._key_counter) <= self.capacity:
            return None
        keys = list(self._key_counter)
        evictable_keys = keys[:-1] or keys
        return min(evictable_keys, key=self._key_counter.__getitem__)
...
class CachedProperty[V]:
    def __init__(self, func: Callable[[HasCache[Any, Any]], V]) -> None:
        self._func = func
    ...
    def __get__(self, instance: HasCache[Any, Any], owner: type) -> V:
        if instance.cache.exists(self._key):
            return cast("V", instance.cache.get(self._key))
